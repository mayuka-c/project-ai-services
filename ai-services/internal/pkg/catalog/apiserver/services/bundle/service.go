...
func (s *bundleService) ProcessBundle(ctx context.Context, file io.Reader, userID string) (*BundleResponse, error) {
	...
	if err := s.repo.Activate(ctx, bundle.ID, meta.Version, name, sizeBytes); err != nil {
...
func (s *bundleService) ReplaceBundle(ctx context.Context, existing *BundleRecord, file io.Reader, userID string) (*BundleResponse, error) {
	...
	go s.runReplaceAsync(existing.ID, existing.Name, newName, meta.Version, existing.CatalogType, existing.CatalogID, archiveBytes)
...
func (s *bundleService) runReplaceAsync(bundleID, oldName, newName, version, catalogType, catalogID string, archiveBytes []byte) {
	ctx := context.Background()

	destDir := bundleDirPath(catalogType, newName)

	if err := os.MkdirAll(destDir, 0o750); err != nil {
		return
	}

	sizeBytes, err := extractAndMeasure(bytes.NewReader(archiveBytes), destDir, catalogID)
	if err != nil {
		os.RemoveAll(destDir)
		return
	}
	...
	if err := s.repo.Activate(ctx, bundleID, version, newName, sizeBytes); err != nil {
		os.RemoveAll(destDir)
		return
	}
	...
	if oldName != "" && oldName != newName {
		os.RemoveAll(bundleDirPath(catalogType, oldName))
	}
